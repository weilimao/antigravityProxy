/**
 * Antigravity Proxy - Chart Renderer Component
 */
...
function getFilteredTrends(trends, range) {
    ...
    if (range === 'today') {
        ...
        if (!state.customStartDate || !state.customEndDate) {
            ...
            slots = generateCustomSlots(state.customStartDate, state.customEndDate);
        }
    } else {
        slots = generateHourlySlots(168); // Fallback 7d
    }
...
function initChartFilters() {
    ...
    if (!chartRangeSelector || !chartFilterPanel || !btnCancelFilter || !btnApplyFilter) return;
    ...
    btnApplyFilter.addEventListener('click', () => {
        ...
        state.customStartDate = new Date(`${startD}T${startT}`);
        state.customEndDate = new Date(`${endD}T${endT}`);
